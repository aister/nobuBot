...
exports.exec = (bot, message, msgArray, callback) => {
  if (message.mentions.users.size > 0) {
    ...
    if (message.guild && message.guild.members.get(mentions.id) && message.guild.members.get(mentions.id).nickname) message.channel.sendFile("http://moesocial.com/slap.php?text=" + encodeURI(message.guild.members.get(mentions.id).nickname), "slap.png").then(callback);
