    ...
    request('http://www.bing.com/images/search?q=' + encodeURI(searchTerm), function(err, res, body) {
      if (err) callback(err);
        ...
        if (body.indexOf('class="thumb" ') > -1) {
          ...
          message.channel.sendFile(body, "image.png", "First image found for query: " + searchTerm).then(callback);
        } else message.channel.sendMessage("There is no image found for query " + searchTerm).then(callback);
      }
    });
  });
}
