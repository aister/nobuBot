...
exports.exec = (bot, message, msgArray, callback) => {
  if (msgArray.length > 2) {
    ...
    message.channel.send('Searching Wikia...').then(msg => {
      request({ url: msgArg, json: true, followRedirect: false }, function(err, res, body) {
        if (res.statusCode != 302 && body.items) {
          ...
          request({ url: msgArg, json: true }, function(err, res, body2) {
            ...
            if (body2.thumbnail) {
              msg.delete();
              message.channel.send(body2.abstract.replace(/\[view\] \u2022 \[talk\] /g, "") + "\n\n<http://" + msgArray[1] + ".wikia.com" + body2.url + ">", {file: {attachment: body2.thumbnail.replace(/\/revision.+/g, ''), name: "image.png"}}).then(callback);
